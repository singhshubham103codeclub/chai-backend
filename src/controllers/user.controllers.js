...
const registerUser=asyncHandler(async(req,res)=>{
   ...
   const exitedUser= User.findOne({//This looks in your MongoDB collection (the User model) and tries to find one user that matches the condition.
   ...
   const createdUser = await User.findById(user._id).select(//The select() method tells MongoDB which fields to include/(-)exclude.
      "_-assword" -"refereshToken:"//User.findById(user._id).Looks up a user in MongoDB by their _id.finds the user who just registered.
    );

   if(!createdUser){ // this error from my means server side thats why we throw menualy 
